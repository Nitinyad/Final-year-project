...
const Form = () => {
  ...
  const register = async (values, onSubmitProps) => {
    try {
      ...
      // Read response as text first to debug empty responses
      const responseText = await savedUserResponse.text();
      ...
      let savedUser;
      try {
        savedUser = JSON.parse(responseText);
      } catch (error) {
      ...
      if (savedUser) {
        ...
        console.log("Oombi myr (Something went wrong)");
      }
    } catch (error) {
  ...
  const resetPasswordSecurity = async (values, onSubmitProps) => {
    const response = await fetch("http://localhost:6001/auth/reset-password-security", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(values),
    });
